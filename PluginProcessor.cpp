...
MakoDist01AudioProcessor::MakoDist01AudioProcessor()
    ...
    {       
        ...
        std::make_unique<juce::AudioParameterInt>("amp_compf","Amp Comp Freq", 50, 3000, 50),
        ...
        std::make_unique<juce::AudioParameterInt>("amp_airf","Amp Air F", 500, 2500, 1500),
        ...
        std::make_unique<juce::AudioParameterFloat>("mod1_regen","Mod 1 Regen", .0f, 1.0f, .3f),
        ...
        std::make_unique<juce::AudioParameterFloat>("mod2_mix","Mod 2 Mix", .0f, 1.0f, .5f),      //R5.02 Changed.
        ...
        std::make_unique<juce::AudioParameterFloat>("mod3_rate","Mod 3 Rate", .0f, 1.0f, .2f),
        ...
        std::make_unique<juce::AudioParameterFloat>("mod4_foff","Mod 4 Foff", -1.0f, 1.0f, .1f),
        ...
        std::make_unique<juce::AudioParameterFloat>("comp_att","Comp Att", .0f, 1.0f, .5f),  //R5.00 Changed.
        std::make_unique<juce::AudioParameterFloat>("comp_rel","Comp Rel", .0f, 1.0f, .2f),
        std::make_unique<juce::AudioParameterFloat>("comp_thr","Comp Thr", .0f, 1.0f, .4f),
        std::make_unique<juce::AudioParameterFloat>("comp_rat","Comp Rat", .0f, 1.0f, .3f),
        std::make_unique<juce::AudioParameterFloat>("comp_drv","Comp Drv", .0f, 1.0f, .0f),
        std::make_unique<juce::AudioParameterInt>("comp_freq","Comp Freq", 50, 4000, 4000),
        std::make_unique<juce::AudioParameterFloat>("comp_mix", "Comp Mix", .0f, 1.0f, .75f),

        //R5.02 Changed defaults.
        ...
        std::make_unique<juce::AudioParameterInt>("dist1_midf","Dist 1 MidF", 200, 2200, 700),   
        ...
        std::make_unique<juce::AudioParameterInt>("dist1_cut","Dist 1 Cut", 2000, 7000, 2000),
        std::make_unique<juce::AudioParameterFloat>("dist1_drv","Dist1 Drv", .0f, 1.0f, .4f),
        ...
        std::make_unique<juce::AudioParameterFloat>("dist2_high", "Dist 2 High", -1.0f, 1.0f, .0f), //R5.02 Changed.
...
{     
    ...
    Setting[e_AmpAirF] = parameters.getRawParameterValue("amp_airf");
    ...
    Setting[e_Mod1Reg] = parameters.getRawParameterValue("mod1_regen");
    ...
    Setting[e_CompFreq] = parameters.getRawParameterValue("comp_freq"); //R5.05 Added.
    Setting[e_CompMix] = parameters.getRawParameterValue("comp_mix");   //R5.06 Added.
...
//R5.04 Apply gain to a High Pass filter then mix in with the original audio signal.
float MakoDist01AudioProcessor::Mako_Amp_Air(float tSample, int channel)
{
    float tS;
    float tStore;
        
    tS = Filter_Calc_BiQuad(tSample, channel, &fil_Amp_Air);
    tS = tanhf(tS * *Setting[e_AmpAir] * 4.0f);

    //R5.04 Added low pass filtering to soften aliasing.
    if (InputEQ_4xOS)
    {
        tStore = tS;
        tS = (tS + Air_Last[channel]) * .5f;
        Air_Last[channel] = tStore;
    }   
...
{   
    ...
    if (DefAmp_Reload == 1)
    {
        ...
        makoEditorNeedsUpdated = true;  //R5.06 Flag editor to rebuild its amp lists.
    }
    ...
    //R5.03 Added AIR adjustment.
    if (SettingRqd[e_AmpAirF])
    {
        Mako_UpdateLastVar(e_AmpAirF);        
        Filter_HP_Coeffs(*Setting[e_AmpAirF], &fil_Amp_Air);        
    }
    ...
    if (SettingRqd[e_AmpComp])
    {
        ...
        Amp_Comp_Threshold = *Setting[e_AmpComp] * *Setting[e_AmpComp] * *Setting[e_AmpComp] * *Setting[e_AmpComp]; //R5.02 Added.
    }
    ...
    //**********************************************************
    //R5.06 PEDAL SETTINGS - Updated.
    //**********************************************************
    if (SettingRqd[e_GateAvg]) Mako_FX_Setup_NoiseGate(1, e_GateAvg);
    if (SettingRqd[e_GateFreq])  Mako_FX_Setup_NoiseGate(1, e_GateFreq);
    if (SettingRqd[e_GateWahRange]) Mako_FX_Setup_NoiseGate(1, e_GateWahRange);

    if (SettingRqd[e_CompThr]) Mako_FX_Setup_Compressor(2, e_CompThr); 
    if (SettingRqd[e_CompAtt]) Mako_FX_Setup_Compressor(2, e_CompAtt);   //R5.06 Added.
    if (SettingRqd[e_CompRel]) Mako_FX_Setup_Compressor(2, e_CompRel);   //R5.06 Added.
    if (SettingRqd[e_CompMix]) Mako_FX_Setup_Compressor(2, e_CompMix);   //R5.06 Added.
    if (SettingRqd[e_CompFreq]) Mako_FX_Setup_Compressor(2, e_CompFreq); //R5.06 Added.

    if (SettingRqd[e_Dist1Mid]) Mako_FX_Setup_Distortion1(3, e_Dist1Mid); 
    if (SettingRqd[e_Dist1MidF]) Mako_FX_Setup_Distortion1(3, e_Dist1MidF);
    if (SettingRqd[e_Dist1MidQ]) Mako_FX_Setup_Distortion1(3, e_Dist1MidQ);
    if (SettingRqd[e_Dist1Cut]) Mako_FX_Setup_Distortion1(3, e_Dist1Cut);
    
    if (SettingRqd[e_Dist2Mid])   Mako_FX_Setup_Distortion2(4, e_Dist2Mid); 
    if (SettingRqd[e_Dist2MidF])  Mako_FX_Setup_Distortion2(4, e_Dist2MidF); 
    if (SettingRqd[e_Dist2MidQ])  Mako_FX_Setup_Distortion2(4, e_Dist2MidQ); 
    if (SettingRqd[e_Dist2High])  Mako_FX_Setup_Distortion2(4, e_Dist2High); 
    if (SettingRqd[e_Dist2Cut])   Mako_FX_Setup_Distortion2(4, e_Dist2Cut); 
    
    //R5.06 EQ Settings.
    if (SettingRqd[e_EQF1F]) Mako_FX_Setup_EQ(5, e_EQF1F); 
    if (SettingRqd[e_EQF1A]) Mako_FX_Setup_EQ(5, e_EQF1A); 
    if (SettingRqd[e_EQF1Q]) Mako_FX_Setup_EQ(5, e_EQF1Q); 
    if (SettingRqd[e_EQF2F]) Mako_FX_Setup_EQ(5, e_EQF2F); 
    if (SettingRqd[e_EQF2A]) Mako_FX_Setup_EQ(5, e_EQF2A); 
    if (SettingRqd[e_EQF2Q]) Mako_FX_Setup_EQ(5, e_EQF2Q); 
    if (SettingRqd[e_EQF3F]) Mako_FX_Setup_EQ(5, e_EQF3F); 
    if (SettingRqd[e_EQF3A]) Mako_FX_Setup_EQ(5, e_EQF3A); 
    if (SettingRqd[e_EQF3Q]) Mako_FX_Setup_EQ(5, e_EQF3Q); 
    if (SettingRqd[e_EQF4F]) Mako_FX_Setup_EQ(5, e_EQF4F); 
    if (SettingRqd[e_EQF4A]) Mako_FX_Setup_EQ(5, e_EQF4A); 
    if (SettingRqd[e_EQF4Q]) Mako_FX_Setup_EQ(5, e_EQF4Q);   

    //**********************************************************
    //R5.06 MODULATION SETTINGS
    //**********************************************************
    //R5.06 DEPTH settings.
    if (SettingRqd[e_Mod1Depth]) Mako_FX_Setup_Mod1(e_Mod1Depth);
    if (SettingRqd[e_Mod2Range]) Mako_FX_Setup_Mod2(e_Mod2Range); 
    if (SettingRqd[e_Mod2Rate])  Mako_FX_Setup_Mod2(e_Mod2Rate); 
    if (SettingRqd[e_Mod3Depth]) Mako_FX_Setup_Mod3(e_Mod3Depth); 
    if (SettingRqd[e_Mod3Bal]) Mako_FX_Setup_Mod3(e_Mod3Bal);
    if (SettingRqd[e_Mod4Foff])  Mako_FX_Setup_Mod4(e_Mod4Foff); 
    if (SettingRqd[e_Mod4Doff])  Mako_FX_Setup_Mod4(e_Mod4Doff);
    if (SettingRqd[e_Mod4Bal]) Mako_FX_Setup_Mod4(e_Mod4Bal); 
...
void MakoDist01AudioProcessor::Mako_Program_Init()
{
    ...
    Mako_FX_Setup_Mod1(e_Mod1Rate);               //R5.06 Changed.
    ...
    Mako_FX_Setup_Mod3(e_Mod3Vol);             //R5.06 Changed.
    ...
    Filter_HP_Coeffs(1500.0f, &fil_Amp_Air);                   //R5.03 AIR
    Filter_BP_Coeffs(0.0f, 50.0f, Amp_Comp_Q, &fil_Amp_Comp);  //R4.30 COMP Setting[e_AmpCompF]. Initialize to lowest value always!
    Mako_EQ_Set_0();

    //R5.06 PreCalc the Compressor filter.
    Filter_BP_Coeffs(12.0f, 100.0f, .35f, &fil_Comp_Freq);
...
{   
    ...
    //R5.02 Calc Over Sampling filters.
    Filter_LP_Coeffs_4xOS(10000.0f, &fil_AmpOverSamp);
    Filter_LP_Coeffs_4xOS(10000.0f, &fil_DistOverSamp);
    Filter_LP_Coeffs_4xOS(10000.0f, &fil_DistOverSamp2);
    Filter_LP_Coeffs_4xOS(10000.0f, &fil_DistOverSamp3);
    Filter_LP_Coeffs_4xOS(10000.0f, &fil_DistOverSamp4);
    ...
    //R5.06 Setup EQ Pedal - Use generic index.
    Mako_FX_Setup_EQ(5, e_EQVol);
...
void MakoDist01AudioProcessor::Mako_Program_Update(bool ForceUpdate)
{
    ...
    //R2.00 Pre Calc our AMPLIFIER EQs.
    Filter_LP_Coeffs(*Setting[e_AmpLP], &fil_Amp_LP);
    Filter_HP_Coeffs(*Setting[e_AmpHP], &fil_Amp_HP); //R5.03 Bug.
...
void MakoDist01AudioProcessor::Mako_ExternalData_Load(bool ForceUpdate)
{
    ...
    DefAmp_File[6] = "*DLX"; DefAmp_GainSetting[6] = 1;         DefAmp_PreGain[6] = true;
...
void MakoDist01AudioProcessor::Mako_Amp_Set()
{
    ...
    if (Idx < 20)
    {
        ...
        case 5://R4.00 CL30
            ...
            InputEQ_PreEQ = true;
            break;
        case 6://R5.03 Added DLX.
            for (int t = 0; t < 1024; t++) InputEQ[t] = InputEQ_DLX[t];
            InputEQ_IRGain = .05f;
            InputEQ_GainRange = 10000.0f;
            InputEQ_PreEQ = true;
            break;
    ...
    //R5.06 Flag the editor it needs to update the PRE EQ indicator.
    makoEditorNeedsUpdated = true;
}
...
void MakoDist01AudioProcessor::Mako_FX_Setup_Mod1(int eIndex)
{        
    Mako_UpdateLastVar(eIndex); //R5.06 Added.
...
void MakoDist01AudioProcessor::Mako_FX_Setup_Mod2(int eIndex)
{
    Mako_UpdateLastVar(eIndex); //R5.06 Added.
...
//R5.06 Initialize the Mod3 Chorus effect.
void MakoDist01AudioProcessor::Mako_FX_Setup_Mod3(int eIndex)
{
    Mako_UpdateLastVar(eIndex); //R5.06 Added.
...
//R5.06 Initialize the Mod4 Wide Load effect.
void MakoDist01AudioProcessor::Mako_FX_Setup_Mod4(int eIndex)
{
    Mako_UpdateLastVar(eIndex); //R5.06 Added.

    //R4.00 Apply FREQ Offset by making a stereo Mid F adjustment.
    Filter_BP_Coeffs((-*Setting[e_Mod4Foff]) * 6.0f, 500.0f, .35f, &filMod4[0]);
    Filter_BP_Coeffs((*Setting[e_Mod4Foff]) * 6.0f, 500.0f, .35f, &filMod4[1]);
...
float MakoDist01AudioProcessor::Mako_Amp_InputIR_4x(float tSample, int channel)
{
    ...
    if (0.0f < *Setting[e_AmpGain])
    {
        ...
        //R5.02 Moved interpolated samples up by 1.
        for (int SampX = 1; SampX < 5; SampX++)
        {
    ...
    //R5.02 Store the last filtered (IR) dry sample to interpolate with.
...
void MakoDist01AudioProcessor::Mako_FX_Setup_NoiseGate(int pos, int eIndex)
{
    Mako_UpdateLastVar(eIndex);
...
void MakoDist01AudioProcessor::Mako_FX_Setup_Compressor(int pos, int eIndex)
{
    Mako_UpdateLastVar(eIndex);
    ...
    //R5.06 PreCalc a MIX variable for speed.
    makoPedal[pos].Var4[0] = 1.0f - *Setting[e_CompMix];

    //R5.06 PreCalc the ATTACK adjusting math.
    makoPedal[pos].Var5[0] = 1.0f + ((1.0f - *Setting[e_CompAtt]) * 20.0f);

    //R5.06 PreCalc the ATTACK adjusting math.
    makoPedal[pos].Var6[0] = 1.0f + ((1.0f - *Setting[e_CompRel]) * 10.0f);

    //R5.06 Setup the Trigger Detection filter. Test if needed.
    if (eIndex == e_CompFreq) Filter_BP_Coeffs(12.0f, *Setting[e_CompFreq], .35f, &fil_Comp_Freq);


}

//R5.06 Pedal 3 - DISTORTION 1 frequency definitions.
void MakoDist01AudioProcessor::Mako_FX_Setup_Distortion1(int pos, int eIndex)
{
    Mako_UpdateLastVar(eIndex);
...
//R5.06 Pedal 4 - DISTORTION 2 frequency definitions.
void MakoDist01AudioProcessor::Mako_FX_Setup_Distortion2(int pos, int eIndex)
{
    Mako_UpdateLastVar(eIndex);

    //R5.02 Changed Bright to after gain and changed its gain to +/- boost.
    //R5.02 Added Low Cut (Filter 4).
    ...
    Filter_BP_Coeffs(*Setting[e_Dist2High] * 12.0f, 1800.0f, .35f, &makoPedal[pos].Filter3); 
    Filter_HP_Coeffs(150.0f, &makoPedal[pos].Filter4);
}

//R4.00 Pedal 5 - STUDIO EQ frequency definitions.
void MakoDist01AudioProcessor::Mako_FX_Setup_EQ(int pos, int eIndex)
{
    Mako_UpdateLastVar(eIndex);
...
float MakoDist01AudioProcessor::Mako_Amp_Comp(float tSample, int channel)
{
    ...
    {
        //R5.02 Get Difference in Signal and Threshold.    
        ...
        else
        {
            ...
            Amp_Comp_Var1[channel] += (Release_500mS * 5.0f); //R4.30 was 18.0f.
    ...
    //R5.02 AMP COMP/SAG. Use compress value 0 - 1 for filter gain adjust. Using linear vals for speed. 
    Filter_BP_Coeffs_AmpSag(Amp_Comp_Var1[channel], &fil_Amp_Comp); 
    return Filter_Calc_BiQuad(tSample, channel, &fil_Amp_Comp);    
...
{
    float tThresh = makoPedal[pos].Var3[0]; //R5.06 Threshold is preCalced in setup for speed.
    float diff;
    float tSa;
    float tS;  
    ...
    //R5.05 Filter the detection signal.
    if (*Setting[e_CompFreq] < 4000.0f)
    {        
        tSa = Filter_Calc_BiQuad(tSa, channel, &fil_Comp_Freq);
        tSa = tSa * LinearGain_12dB_Inv;  //R5.06 Need to offset the 12dB boost in filter.
    }

    //R5.05 Get absolute value.
    tSa = abs(tSa);
    ...
    if (tThresh < tSa)
    {
        ...
        if (makoPedal[pos].Var2[channel] < makoPedal[pos].Var1[channel])
        {
            ...
            //R5.06 VAR5 is the ATTACK adjustment math, preCalc in setup.
            makoPedal[pos].Var1[channel] -= (Release_500mS * makoPedal[pos].Var5[0]);
            if (makoPedal[pos].Var1[channel] < 0.0f) makoPedal[pos].Var1[channel] = 0.0f;
        }
        else
        {
            //R1.00 Reduce the Gain adjust back to 1.0f.
            //R5.06 VAR6 is the RELEASE adjustment math, preCalc in setup.
            makoPedal[pos].Var1[channel] += (Release_500mS * makoPedal[pos].Var6[0]);
    ...
    //R5.06 Apply GAIN and DRIVE if needed.
    if (*Setting[e_CompDrv] < .0001f)
        tS = tSample * makoPedal[pos].Var1[channel] * *Setting[e_CompVol] * 4.0f;
    else
        tS = tanh(tSample * makoPedal[pos].Var1[channel] * (1.0f + (*Setting[e_CompDrv] * 10.0f))) * *Setting[e_CompVol] * 2.0f;

    //R5.06 Mix the clean and compressed signals. Var4 is preCalc of (1-Mix).
    return (tSample * makoPedal[pos].Var4[0]) + (tS * *Setting[e_CompMix]);
...
float MakoDist01AudioProcessor::Mako_FX_Audio_Distortion2(float tSample, int channel, int pos)
{
    ...
    //R5.02 Added Low Cut here to remove some bass before gain.
    tS = Filter_Calc_BiQuad(tS, channel, &makoPedal[pos].Filter4);

    //R2.00 BP - Mid/Bright boost.
    tS = Filter_Calc_BiQuad(tS, channel, &makoPedal[pos].Filter1);
    //tS = Filter_Calc_BiQuad(tS, channel, &makoPedal[pos].Filter3); //R5.02 moved.

    //R5.02 This sample needs to be stored for interpolation on the next sample.
    tS_PostEQ = tS;
    
    //R5.02 Apply gain and clipping if gain is not set to Zero.
    if (1.0f < Gain)
    {
    ...
    //R5.02 Store the last filtered (IR) dry sample to interpolate with.
    Dist2_Sample_Last = tS_PostEQ;

    //R5.02 Moved Bright to AFTER gain. This combined with Fizz filter gives more adjustment.
...
float MakoDist01AudioProcessor::Mako_FX_Audio_Chorus(float tSample, int channel)
{
    ...
    //R5.02 Alter our delay direction if needed.
    if (Mod3_B_Offset[channel] < Mod3_B_Offset_Min[channel])
        Mod3_B_Rate[channel] = Mod3_B_Rate_Up * (.5f + (4.0f * *Setting[e_Mod3Depth]));   //R5.02 Added Depth adjust.
    else if (Mod3_B_Offset_Max[channel] < Mod3_B_Offset[channel])
        Mod3_B_Rate[channel] = Mod3_B_Rate_Down * (.5f + (4.0f * *Setting[e_Mod3Depth])); //R5.02 Added Depth adjust.
...
float MakoDist01AudioProcessor::Mako_FX_Audio_ModWah(float tSample, int channel)
{
    ...
    //R5.02 Calc filter coeffs. This is expensive and should be optimized more if possible. Like call it every X samples.
...
float MakoDist01AudioProcessor::Mako_FX_Audio_Flange(float tSample, int channel)
{
    ...
    Mod1_B[channel][Mod1_B_Ring1[channel]] = (tS - (tS_Mod * *Setting[e_Mod1Reg])) * .5f;
    ...
    {
        //R5.06 Fixed rectifier bug.
        tS = (tS * (1.0f - *Setting[e_Mod1Bronz])) + (2.0f * abs(tS) * *Setting[e_Mod1Bronz]);
...
float MakoDist01AudioProcessor::Mako_FX_ReverbFlange(float tSample, int channel)
{
    ...
    {
        RevMod_B_Offset[channel] = 0.0f;              //R5.02 Added
        RevMod_B_Rate[channel] = RevMod_B_Rate_Up;
    }
    else if (Rev_Flange_Limit < RevMod_B_Offset[channel])
    {
        RevMod_B_Offset[channel] = Rev_Flange_Limit;  //R5.02 Added
        RevMod_B_Rate[channel] = RevMod_B_Rate_Down;
    }
