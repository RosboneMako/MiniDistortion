...
{   
    ...
    //R5.05 Added Compressor filter detection.
    if (SettingRqd[e_CompFreq])
    {
        Mako_UpdateLastVar(e_CompFreq);
        Filter_BP_Coeffs(12.0f, *Setting[e_CompFreq], .35f, &fil_Comp_Freq);
    }
...
float MakoDist01AudioProcessor::Mako_FX_Audio_Compressor(float tSample, int channel, int pos)
{
    ...
    {        
        ...
        tSa = tSa * .1f;  //R5.05 Need to offset the 12dB boost in filter.
    }
